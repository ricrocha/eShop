...
> 4. **Extensibility Contract** — when adding a new API service or third-party integration, state the mandatory steps: register in `eShop.AppHost`, add service defaults, expose via the established Minimal API pattern, publish/subscribe through the event bus, and reflect any required UI changes in `WebApp` or `WebAppComponents`; new integrations must not bypass the event bus or introduce direct service-to-service HTTP calls outside of service discovery
...
> 16. **Testing** — MSTest with `Microsoft.Testing.Platform`, NSubstitute (never Moq), Arrange/Act/Assert structure, parallel execution, direct handler unit testing vs `WebApplicationFactory` for functional tests, GlobalUsings.cs convention in test projects
